...
class LMDriver{
    ...
    public:
    LMDriver(PinName msoi, PinName miso, PinName sclk,PinName chip_select=dp24);
    ...
    void SetWaitTime(float time, bool test=true) { if(test) wtime=time; else cwtime=time; }
    void operator=(char c) {DisplayChar(c);}
    void operator=(const string& str){  DisplayStringCont(str); }
    void Clear() {zeroFill();}
